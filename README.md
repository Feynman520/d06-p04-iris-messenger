...
7. 파일은 클립 단추로 고른 뒤 **보내기(Enter)** 를 눌러야 나갑니다(글을 함께 적으면 파일 다음에 글이 갑니다). 받은 파일은 카드의 「받기」를 누르면 IRIS 폴더 밑 **`_messenger\<상대 이름>\`** 에 원래 이름으로 저장됩니다(IRIS 폴더를 못 찾는 설치에서는 모듈의 `state\files\`).
