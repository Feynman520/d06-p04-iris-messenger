...
try {
  // 만들자마자 정리 목록에 올린다 — 로그인(adminSignIn)이 실패해도 계정이 남지 않게.
  const mk = async (n) => {
    const email = `rls-${n}-${ts}@example.com`;
    const { id } = await adminCreateUser(email);
    const u = { id, email, n, token: null };
    users.push(u);
    const s = await adminSignIn(email);
    u.token = s.access_token;
    return u;
  };
  ...
  objects.push(`${A.id}/${fid}`); // 올라갔든 아니든 정리 목록에 둔다(없는 객체 삭제는 조용히 지나간다)
