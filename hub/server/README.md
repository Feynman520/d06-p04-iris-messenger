...
2. **Management API PAT(개인 액세스 토큰) 발급 → `.env`** — Supabase 대시보드 → Account → Access Tokens에서 새 토큰 발급 후 `.env`에 `SUPABASE_ACCESS_TOKEN=<토큰>` 한 줄을 넣는다. Supabase 계정을 여럿 굴린다면 `SUPABASE_ACCESS_TOKEN_<계정>` 형태(계정 이름은 `hub-ref.json`의 `account` 값과 같게)로도 읽는다 — 이 저장소 저자는 `SUPABASE_ACCESS_TOKEN_FEYNMAN520`을 쓴다. Storage 정리·라이브 검사에 쓰는 service role key는 같은 파일의 `SUPABASE_SERVICE_ROLE_KEY`(저자 예: `SUPABASE_SERVICE_ROLE_KEY_IRIS_MESSENGER`)로 둔다.
   - **`.env` 위치**: 환경변수 `IRIS_MESSENGER_ENV`에 적은 경로 → (저자의 개인 경로) → 없으면 **이 저장소 루트의 `.env`**. 즉 다른 운영자는 저장소 루트에 `.env` 하나만 두면 되고, `.gitignore`가 그 파일을 커밋에서 막는다.
3. **`hub-ref.json` 작성** — 이 폴더에 `hub-ref.json`을 만든다: `{ "ref": "<project ref>", "url": "https://<ref>.supabase.co", "account": "<PAT 이름에 쓸 계정>" }`. (커밋해도 되는 공개 정보 — 공개 식별자일 뿐 비밀이 아니다. `account`는 위 2번의 계정별 PAT 이름을 찾는 데만 쓴다.)
...
메타데이터(마지막 줄)는 서버 운영에 필요한 최소 정보로, 자세한 사항은 `처리방침.md`에 명시한다.
