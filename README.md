...
5. 코드를 입력하면 상대의 이름과 **지문(공개 열쇠를 8자리로 줄인 값)**이 뜹니다. **반드시 상대와 지문을 직접 비교(전화·대면 등으로 불러주기)한 뒤에 수락하세요** — 이 대조를 건너뛰면 서버가 열쇠를 바꿔치기하는 공격(MITM)을 알아챌 방법이 없습니다.
...
- **지문은 8글자(약 40비트)입니다.** 짧아서 눈으로 비교하기 쉽지만, 그만큼 완전한 충돌 회피를 보장하는 길이는 아닙니다 — 실제 방어력은 "지문을 실제로 대조하는 습관"에 달려 있습니다.
