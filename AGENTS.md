...
- 구조: `module\`(Face `modules\messenger\`에 복사되는 클라이언트: 화면·배지·파일 첨부) · `hub\server\`(Supabase 표·RLS 정책·청소·셋업 README) · `hub\client\`(로그인·토큰 보관·E2EE 열쇠·연결·연락처 공용 라이브러리) · `docs\`(설계·검증 기록). **`hub\`는 `module\`을 참조하지 않는다**(반대 방향만) — 다음 모듈이 생기면 `hub\`를 통째로 새 P로 떼어낸다.
